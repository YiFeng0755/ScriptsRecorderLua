...
//        newShell.setText("脚本录制工具V0.9");
        newShell.setText("脚本录制工具V0.9.1");
    }
    ...
    public void setModel(final UiAutomatorModel model, final File uiXml,
                                                                final Image screenshot) {
        if (Display.getDefault().getThread() != Thread.currentThread()) {
            Display.getDefault().syncExec(new Runnable() {
                @Override
                public void run() {
                    mUiAutomatorView.setModel(model, uiXml, screenshot);
                }
            });
        } else {
            mUiAutomatorView.setModel(model, uiXml, screenshot);
