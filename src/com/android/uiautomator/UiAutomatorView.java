...
public class UiAutomatorView extends Composite {
	...
	// The screenshot area is made of a stack layout of two components:
	// screenshot canvas and
	// a "specify screenshot" button. If a screenshot is already available, then
	// that is displayed
	// on the canvas. If it is not availble, then the "specify screenshot"
	// button is displayed.
	...
	public UiAutomatorView(Composite parent, int style) {
		...
		mScreenshotCanvas
				.setMenu(createEditPopup(mScreenshotCanvas.getShell()));
		...
		mRIListViewer = new ListViewer(mRunningInfoComposite, SWT.V_SCROLL
				| SWT.H_SCROLL);
		...
		mRIListViewer.setContentProvider(new IStructuredContentProvider() {
			...
			public void inputChanged(Viewer v, Object oldO, Object newO) {
				System.out
						.println("runing info list view content changed, new info:"
								+ newO);
			}
		});
		...
		mRIListViewer.setSorter(new ViewerSorter() {
			public int compare(Viewer v, Object o1, Object o2) {
				return ((RunningInfo) o1).getThreadName().compareTo(
						((RunningInfo) o1).getThreadName());
			}
		});
		mRIListViewer.getList().setLayoutData(
		...
		toolBarManager.add(new FinishRecordAction(this));// finsh record and
															// save the script
															// button
		toolBarManager.add(new RefreshAction(this));// refresh and repaint the
													// current page
		toolBarManager.add(new AddSleepAction(this));// when a page need some
														// time to load, click
														// this to add a time to
														// wait
		...
		mListViewer.addDoubleClickListener(new IDoubleClickListener() {
			
			@Override
			public void doubleClick(DoubleClickEvent arg0) {
				// TODO Auto-generated method stub
				StructuredSelection ssl = (StructuredSelection) mListViewer.getSelection();
				Action ac = (Action) ssl.getFirstElement();
				actions.remove(ac);
		...
		mListViewer.setContentProvider(new IStructuredContentProvider() {
			...
			public void inputChanged(Viewer v, Object oldO, Object newO) {
				System.out.println("list view input changed,from " + oldO
						+ "to " + newO);
			}
		});
		...
		mListViewer.setLabelProvider(new LabelProvider() {
			...
			public String getText(Object element) {
				if (Operate.CLICK.equals(((Action) element).getType())) {
					return ((Action) element).getType() + ">>"
							+ ((Action) element).getItemName();
				} else if (Operate.INPUT.equals(((Action) element).getType())) {
					return ((Action) element).getType() + ">>"
							+ ((Action) element).getItemName() + "[|]"
							+ ((Action) element).getOperation().split("[|]")[1];
		...
		mListViewer.setSorter(new ViewerSorter() {
			public int compare(Viewer v, Object o1, Object o2) {
				return ((Action) o1).getOperation().compareTo(
						((Action) o1).getOperation());
			}
		});
		mListViewer.getList().setLayoutData(
				new GridData(SWT.FILL, SWT.FILL, true, true, 1, 1));
	...
	public void performAction(Action action) {
		try {
			...
			if (Operate.CLICK.equals(action.getType())) {
				...
				String bound = com.jack.utils.StringUtil.getSubString(itemName,
						"\\[.*\\]\\[.*\\]");
				String[] points = (bound.substring(1, bound.length() - 1))
						.split("\\]\\[");
				int x = (Integer.parseInt(points[0].split(",")[0]) + Integer
						.parseInt(points[1].split(",")[0])) / 2;
				int y = (Integer.parseInt(points[0].split(",")[1]) + Integer
						.parseInt(points[1].split(",")[1])) / 2;
				String command = "input tap " + x + " " + y;
				CountDownLatch commandCompleteLatch = new CountDownLatch(1);
				NewScriptDialog.getsIDevice().executeShellCommand(command,
				...
				System.out.println("\toperation: input "
						+ action.getOperation().split("[|]")[1]);
				String command = "am startservice --user 0 -n com.boyaa.luaviewer_helper/.ViewerHelper"
						+ com.jack.utils.StringUtil.strToIntenExtra(action
								.getOperation());
				CountDownLatch commandCompleteLatch = new CountDownLatch(1);
				NewScriptDialog.getsIDevice().executeShellCommand(command,
						new CollectingOutputReceiver(commandCompleteLatch));
				...
				System.out.println("\toperation:" + action.getOperation());
				System.out.println("\toperation: swipe to"
						+ action.getOperation().split("[|]")[1]);
				...
				System.out.println("\toperation:" + action.getOperation());
				System.out.println("\toperation: send key code "
						+ action.getOperation().split("[|]")[1]);
				String command = "input keyevent KEYCODE_BACK";
				CountDownLatch commandCompleteLatch = new CountDownLatch(1);
				NewScriptDialog.getsIDevice().executeShellCommand(command,
						new CollectingOutputReceiver(commandCompleteLatch));
				commandCompleteLatch.await(5, TimeUnit.SECONDS);
				...
				Thread.sleep(Integer
						.valueOf(action.getOperation().split("[|]")[1]));
				return;
				// AppiumActUtil.sendKeyCode(appiumConfig.getDriver(),
				// action.getOperation().split("[|]")[1]);
			}
		} catch (Exception e) {
			...
			dialog.run(true, true, new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor)
					...
					try {
						...
						UiAutomatorResult result = UiAutomatorHelper
								.takeSnapshot(NewScriptDialog.getsIDevice(),
										monitor);
						MyImageUtil.saveSWTImage(result.screenshot,
								Constant.SAVE);
						// System.out.println("test driver init;currentActivity:"
						// + AppiumConfig.getDriver().currentActivity());
						// setModel(result.model, result.uiHierarchy,
						// result.screenshot);
						...
						ErrorHandler.showError(getShell(),
								"Appium obtain page error", e);
						return;
					} catch (Exception e) {
						e.printStackTrace();
						return;
	 ...
	 * @param arg
	 *            操作参数
	 * @return Action 根据传入信息，构造出的Action对象 2016年11月28日
	 */
	...
	public Menu createEditPopup(Shell parentShell) {
		...
		inputItem.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				 ...
				 * {
				 * ErrorHandler.showError(getShell(),"Error,not a inputable widget"
				 * , new Exception("unsupport input widget")); return; }
				 */
				...
				if (InputDialog.getInput() == null
						|| InputDialog.getInput().trim().equals("")) {
					return;
				}
				performAction(updateAction(mModel.getSelectedNode(),
						Operate.INPUT, InputDialog.getInput()));
			}
		...
		swipeRightItem
				.addSelectionListener(new SwipeSelectionListener("right"));
	...
	public void setModel(UiAutomatorModel model, File uiXml, Image screenshot) {
		mModel = model;
		...
		if (uiXml != null) {
			...
			try {
				brd = new BufferedReader(new FileReader(uiXml));
				String line = "";
	...
	/**
	 * called when a script was finished. 释放当前页面资源，准备下次录制操作
	 * 
	 * @author ZYC 2016年11月28日
	 */
	public void empty() {
		pageSources.clear();
		actions.clear();
